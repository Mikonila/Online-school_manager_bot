...
from bot.fsm.states import FSMPresenter

router = Router()

@router.message(FSMPresenter.presenting)
async def presenter_agent(message: Message, state: FSMContext):
    data = await state.get_data()
    history = data.get("history", "")

    gpt_reply = await ask_gpt(
        user_message=message.text,
        prompt_name="presenter",
        history=history
    )
